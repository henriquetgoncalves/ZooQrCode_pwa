...
selected_animal = gup("animal",location.search);
...
const auth = firebase.auth(),
    ...
    animal = firebase.database().ref('tabelas/animais/'+selected_animal);
...
app.getAnimal = function (key) {
    var url = new URL("./",self.location).href + '/JSONdata/';
    url += key + '.json';
    if ('caches' in window) {
        /*
...
updateAnimalCard = function (data) {
    ...
    app.container.querySelector('#image__animal').style.backgroundImage = getImage("animais",data.key);
